		...
		<div class="container">
			...
			<form:form commandName="searchVO" id="listForm" name="listForm"
				...
				<div id="content_pop" class="mt-5">
					<div id="search"
						class="d-flex flex-wrap justify-content-center py-2">
						<div id="sysbtn"
							class="d-flex align-items-center mb-md-0 me-md-auto text-dark text-decoration-none">
							<a href="javascript:fn_egov_addView();" class="btn btn-primary"><spring:message
					...
					<div id="table">
						<table class="table table-hover mt-2"
							summary="글번호, 제목, 내용, 첨부파일, 등록일, 사용여부 표시하는 테이블">
							<caption style="visibility: hidden">글번호, 제목, 내용, 첨부파일,
								등록일, 사용여부 표시하는 테이블</caption>
							<tr class="bg-light text-center">
								<th>No</th>
								<th style="width: 70%;"><spring:message
										code="title.notice.title" /></th>
							...
							<c:forEach var="result" items="${resultList}" varStatus="status">
								<tr>
									...
									<td class="text-center py-3"><c:if
											...
											<a
												href="<c:url value='/egovAdmin/client/fileDownload.do?fileSaveName=${result.attachSaveName}&fileOriName=${result.attachOriName}'/> ">
									...
									<td class="text-center py-3"><c:out
											value="${result.regDate}" />&nbsp;</td>
									<td class="text-center py-3" style="font-weight: bold;"><c:choose>
											<c:when test="${result.useYn eq 'Y'}">
												<c:out value="${result.useYn}" />&nbsp;
		            					</c:when>
											<c:otherwise>
												<span style="color: red;"><c:out
														value="${result.useYn}" /></span>&nbsp;
		            					</c:otherwise>
										</c:choose></td>
								</tr>
							</c:forEach>
