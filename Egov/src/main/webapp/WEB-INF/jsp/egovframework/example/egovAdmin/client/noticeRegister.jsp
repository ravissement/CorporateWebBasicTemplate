...
<c:set var="registerFlag"
	value="${empty noticeVO.idx ? 'create' : 'modify'}" />
<title>Notice <c:if test="${registerFlag == 'create'}">
...
<validator:javascript formName="noticeVO" staticJavascript="false"
							...
							<tr>
								<td style="font-weight: bold;"
									class="text-center text-secondary"><label for="title"><spring:message
											code="title.notice.title" /></label></td>
								<td><form:input path="title" maxlength="100"
										class="form-control" /> &nbsp;<form:errors path="title" /></td>
							</tr>
							<tr>
								<td style="font-weight: bold;"
									class="text-center text-secondary"><label for="useYn"><spring:message
											code="title.notice.useYn" /></label></td>
								<td><form:select path="useYn" class="form-select w-25">
							...
							<tr>
								<td style="font-weight: bold;"
									class="text-center text-secondary"><label for="contents"><spring:message
											code="title.notice.contents" /></label></td>
								<td><form:textarea path="contents" rows="5" cols="58"
										class="form-control" />&nbsp;<form:errors path="contents" />
								</td>
							</tr>
							<tr>
								<td style="font-weight: bold;"
									class="text-center text-secondary"><label for="regUser"><spring:message
											code="title.notice.regUser" /></label></td>
							...
							<tr>
								<td style="font-weight: bold;"
									class="text-center text-secondary"><label for="attach"><spring:message
											code="title.notice.attach" /></label></td>
								<td><c:if test="${registerFlag == 'modify'}">
										<form:input path="attachFile" type="file"
											class="form-control w-25" />
		        				&nbsp;<form:errors path="attachFile" />
			        				...
			        				<a
												href="<c:url value='/egovAdmin/client/fileDownload.do?fileSaveName=${noticeVO.attachSaveName}&fileOriName=${noticeVO.attachOriName}'/> "
					...
					<div id="sysbtn">
						<ul class="nav justify-content-end">
							...
							<li class="nav-item mx-2"><span> <a
									...
									class="btn btn-info text-light"> <c:if
							...
							<c:if test="${registerFlag == 'modify'}">
								<li class="nav-item mx-2"><span> <a
										href="javascript:fn_egov_delete();" class="btn btn-danger"><spring:message
												code="button.delete" /></a>
								</span></li>
							</c:if>
							<li class="nav-item mx-2"><span> <a
									href="javascript:document.detailForm.reset();"
									class="btn btn-primary"><spring:message code="button.reset" /></a>
							</span></li>
						</ul>
					</div>
				</div>
